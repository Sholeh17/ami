...
<title>REPORT HASIL PENGUJIAN COMPOUND - COMPLITED</title>
<body>
	<div class="div_title"><u>HASIL PENGUJIAN COMPOUND - COMPLITED</u></div>
    ...
    <table width="108%" border="0" cellspacing="2" cellpadding="2">
      <tr>
        ...
        <th colspan="5"><div align="center">PHYSICAL PROPERTIS</div></th>
      ...
      <tr>
        ...
        <th width="5%">HARDNES FRESS<br>(<?php echo $this->mrj->set_set_text_toleransi_param($rowformulir->sample, 11, "Hardenss"); ?></td>
        <th width="5%">Modulus 100 Mpa Fresh<br>(<?php echo $this->mrj->set_set_text_toleransi_param($rowformulir->sample, 11, "M_100mpa"); ?>)</td>
        <th width="5%">Modulus 300 Mpa Fresh<br>(<?php echo $this->mrj->set_set_text_toleransi_param($rowformulir->sample, 11, "M_300mpa"); ?>)</td>
        <th width="5%">Tensile  Mpa Fresh<br>(<?php echo $this->mrj->set_set_text_toleransi_param($rowformulir->sample, 11, "Tensile"); ?>)</td>
      <th width="5%">Elongation % Fresh<br>(<?php echo $this->mrj->set_set_text_toleransi_param($rowformulir->sample, 11, "Elongation"); ?>)</td>        </tr>
      <?php
	  		//print_r($denstie);
			$j = 0; 
			...
			$arr_M_100mpa = array(); $arr_M_300mpa = array(); $arr_Tensile = array(); $arr_Elongation = array();
	  		...
	  		foreach ($rheometer195 as $r) {
				...
				// Physc
				$toleHardenss = $this->mrj->get_toleransi_param_test_report($rowformulir->sample, 11, "Hardenss");
				$statusHardenss = false;
				$Hardenss = (float) $physical[$j]['Hardenss'];
				if ($toleHardenss->start_toleransi <= $Hardenss && $Hardenss <= $toleHardenss->end_toleransi)
					$statusHardenss = true;	
				
				
				if (!isset($physical[$j]['Hardenss']))
					$statusHardenss = true;	
				
				
				$toleM_100mpa = $this->mrj->get_toleransi_param_test_report($rowformulir->sample, 11, "M_100mpa");
				$statusM_100mpa = false;
				$M_100mpa = (float) $physical[$j]['M_100mpa'];
				if ($toleM_100mpa->start_toleransi <= $M_100mpa &&  $M_100mpa <= $toleM_100mpa->end_toleransi)
					$statusM_100mpa = true;
			
				if (!isset($physical[$j]['M_100mpa']))
					$statusM_100mpa = true;	
			
				$toleM_300mpa = $this->mrj->get_toleransi_param_test_report($rowformulir->sample, 11, "M_300mpa");
				$statusM_300mpa = false;
				$M_300mpa = (float) $physical[$j]['M_300mpa'];
				if ($toleM_300mpa->start_toleransi <= $M_300mpa &&  $M_300mpa <= $toleM_300mpa->end_toleransi)
					$statusM_300mpa = true;		
			
				if (!isset($physical[$j]['M_300mpa']))
					$statusM_300mpa = true;	
			
				$toleTensile = $this->mrj->get_toleransi_param_test_report($rowformulir->sample, 11, "Tensile");
				$statusM_Tensile = false;
				$Tensile = (float) $physical[$j]['Tensile'];
				if ($toleTensile->start_toleransi <= $Tensile &&  $Tensile <= $toleTensile->end_toleransi)
					$statusTensile = true;	
				
				if (!isset($physical[$j]['Tensile']))
					$statusTensile = true;	
				
				$toleElongation = $this->mrj->get_toleransi_param_test_report($rowformulir->sample, 11, "Elongation");
				$statusElongation = false;
				$Elongation = (float) $physical[$j]['Elongation'];
				if ($toleElongation->start_toleransi <= $Elongation &&  $Elongation <= $toleElongation->end_toleransi)
					$statusElongation = true;		
					
				if (!isset($physical[$j]['Elongation']))
					$statusElongation = true;	
					
				$statusPhyc = false;
				if ($statusHardenss && $statusM_100mpa && $statusM_300mpa && $statusTensile && $statusElongation)
					$statusPhyc = true;
					
				if ($status_rheo && $statusDENSITY && $statusML_4 && $statusMS && $statusPhyc) 
					$set_status = "OK";
				else
					$set_status = "NG";	
	  ?>
      ...
      <tr>
        ...
        <td><?=$physical[$j]['Hardenss'];?></td>
        <td><?=$physical[$j]['M_100mpa'];?></td>
        <td><?=$physical[$j]['M_300mpa'];?></td>
        <td><?=$physical[$j]['Tensile'];?></td>
        <td><?=$physical[$j]['Elongation'];?></td>
      ...
      <tr align="right"	>
        ...
        <td><?php
			...
			?></td>
        <td><?php
			echo number_format((array_sum($arr_hardenss) / count($arr_hardenss)),2);
			?></td>
        <td><?php
			echo number_format((array_sum($arr_M_100mpa) / count($arr_M_100mpa)),2);
			?></td>
        <td><?php
			echo number_format((array_sum($arr_M_300mpa) / count($arr_M_300mpa)),2);
			?></td>
        <td><?php
			echo number_format((array_sum($arr_Tensile) / count($arr_Tensile)),2);
			?></td>
        <td><?php
			echo number_format((array_sum($arr_Elongation) / count($arr_Elongation)),2);
			?></td>
        <td>&nbsp;</td>
      </tr>
      <tr align="right">
        ...
        <td><?php
			...
			?></td>
        <td><?php
			echo number_format(max($arr_hardenss),2);
			?></td>
        <td><?php
			echo number_format(max($arr_M_100mpa),2);
			?></td>
        <td><?php
			echo number_format(max($arr_M_300mpa),2);
			?></td>
        <td><?php
			echo number_format(max($arr_Tensile),2);
			?></td>
        <td><?php
			echo number_format(max($arr_Elongation),2);
			?></td>
        <td>&nbsp;</td>
      </tr>
      <tr align="right">
        ...
        <td><?php
			...
			?></td>
        <td><?php
			echo number_format(min($arr_hardenss),2);
			?></td>
        <td><?php
			echo number_format(min($arr_M_100mpa),2);
			?></td>
        <td><?php
			echo number_format(min($arr_M_300mpa),2);
			?></td>
        <td><?php
			echo number_format(min($arr_Tensile),2);
			?></td>
        <td><?php
			echo number_format(min($arr_Elongation),2);
			?></td>
        <td>&nbsp;</td>
      </tr>
      <tr align="right">
        ...
        <td><?php
			...
			?></td>
        <td><?php
			echo number_format(max($arr_hardenss) - min($arr_hardenss),2);
			?></td>
        <td><?php
			echo number_format(max($arr_M_100mpa) - min($arr_M_100mpa),2);
			?></td>
        <td><?php
			echo number_format(max($arr_M_300mpa) -  min($arr_M_300mpa),2);
			?></td>
        <td><?php
			echo number_format(max($arr_Tensile) - min($arr_Tensile),2);
			?></td>
        <td><?php
			echo number_format(max($arr_Elongation) -  min($arr_Elongation),2);
			?></td>
        <td>&nbsp;</td>
      </tr>
    </table>
