...
$sql = "SELECT * FROM sms_purchase_order_header a inner join
		...
		#WHERE b.status_sync = 'sms' AND status = 'Approve'";
...
for ($i = 0; $i<count($nopo); $i++) {
	...
	echo $q2 = "INSERT INTO ZTCUSTTMPSALESSMSCONTROL (RECID,PURCHORDERFORMNUM,ISPROCESS,BLANKETID,SALESID) 
			VALUES('".$POrecID[$i]."','".$nopo[$i]."','1','".$nopo[$i]."','".$nopo[$i]."')"; 
	mssql_query($q2);
}
